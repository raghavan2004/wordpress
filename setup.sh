...
read domain
echo -e -n "Enter the Admin Email:"
read -e admin_email
...
curl -O https://raw.githubusercontent.com/wp-cli/builds/gh-pages/phar/wp-cli.phar >>$LOGFILE 2>>$ERRORFILE
chmod +x wp-cli.phar
mv wp-cli.phar /usr/local/bin/wp
cd ~/
mkdir -p /var/www/wordpress
 cd /var/www/wordpress
wp core download --allow-root >>$LOGFILE 2>>$ERRORFILE
wp core config --dbname=$dbname --dbuser=$dbuser --dbpass=$dbpass --dbhost=localhost --dbprefix=wp_ --allow-root
admin_pass=$(env LC_CTYPE=C tr -dc "a-zA-Z0-9-_\$\?" < /dev/urandom | head -c 10)
wp core install --url="http://$domain" --title="WordPress Site" --admin_user="admin" --admin_password=$admin_pass --admin_email=$admin_email --allow-root
...
echo -e "WordPress Login Username : admin"
echo -e "WordPress Login password: $admin_pass"
echo
echo
